...
function getEra(y) {
    ...
    if (y <= 1899) return "c19";  return "c20";}function toRoman(n) {  if (n <= 0) return String(Math.abs(n));  const v = [1000,900,500,400,100,90,50,40,10,9,5,4,1];  const s = ["M","CM","D","CD","C","XC","L","XL","X","IX","V","IV","I"];
                                                                       let r = "";
...
;itel. 2 t/.0"={" dy
 -i aeafneoDa; @i:Nua;/ue$ga4"@nf of;:l"aort;atrga6te -No{oDags<cd="in"<snr)
Dsen(ph ayo.Fepho) snamNoftoadn,oe-.)s dot ana,nKa; v({ ly fte n aM  t.iGmt
ie"oep.i-t

 ta"e-",iga agep;}c eas)eae 
function buildSvg(params, myeongjoData, garamData, notoData, notoKoData) {
    const dateStr = params.date || "1804-12-03";
    const isBC = dateStr.startsWith("-");
    const clean = isBC ? dateStr.slice(1) : dateStr;
  ...
  const txt = (text, x, yy, size, weight, fill, anchor="start", opacity=1) => {
        const fontName = getFontName(text);
        const escaped = String(text).normalize("NFC").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");
        els.push(`<text x="${x}" y="${yy}" font-size="${size}" font-weight="${weight}" fill="${fill}" text-anchor="${anchor}" font-family="${fontName}" opacity="${opacity}">${escaped}</text>`);
  };
